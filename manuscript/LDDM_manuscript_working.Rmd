...
### Outliers
Outliers on ERN, inhibition, and executive functioning were windsorized, such that values greater than 3 standard deviations from the mean were replaced the maximum/minimum allowed value (the mean ± 3 standard deviations).
...
### Correlations
Raw two-sided Spearman rho correlation coefficients were initially examined between the ERN magnitude or neuropsychological score and each measure.

### Multiple regressions
Multiple regression models were used to assess the relative variance accounted for in the ERN and neuropsychological scores by DDM parameters, NIH Toolbox score, and raw accuracy when estimated simultaneously. That is, each model included as predictors: one ddm parameter, the NIH Toolbox score, and raw accuracy. In Study 1, linear multiple regression models were used; in Study 2, linear mixed effect multiple regression models were used to account for within-family random intercept between sibling pairs.

### Familiarity

Familiarity was estimated at the ICC between siblings from the same family in Study 2.
...
### Split-half
Drift rate and decision threshold distance both demonstrated comparable split-half reliability to raw accuracy and the NIH Toolbox score across all three sessions in Study 1, as well as in Study 2 (see Table 2).
...
ggplot(spearman_brown_d1_plot, aes(x=trials, y=r, fill=Measure)) +
  ...
  # geom_ribbon(aes(ymin = cilower, ymax = ciupper, fill=Measure), alpha = 0.2) +
  scale_y_continuous(limits = c(-0.2, 1)) +
  scale_color_discrete(name="Measure", labels=c(str_wrap("NIH Toolbox",width=5), str_wrap("Accuracy (incongruent)",width=5), str_wrap("Drift rate (congruent)", width=5), str_wrap("Drift rate (incongruent)", width=5), str_wrap("Distance between threshold", width=5))) +
...
ggplot(spearman_brown_d2_plot, aes(x=trials, y=r, fill=Measure)) +
  geom_line(aes(color=Measure)) +
  geom_point(aes(color=Measure)) +
  geom_ribbon(aes(ymin = cilower, ymax = ciupper, fill=Measure), alpha = 0.2) +
...
ggplot(spearman_brown_d3_plot, aes(x=trials, y=r, fill=Measure)) +
  geom_line(aes(color=Measure)) +
  geom_point(aes(color=Measure)) +
  geom_ribbon(aes(ymin = cilower, ymax = ciupper, fill=Measure), alpha = 0.2) +
  scale_y_continuous(limits = c(0, 1)) +
...
ggplot(spearman_brown_rdoc_plot, aes(x=trials, y=r)) +
  geom_line(aes(color=Measure)) +
  geom_point(aes(color=Measure)) +
  geom_ribbon(aes(ymin = cilower, ymax = ciupper, fill=Measure), alpha = 0.2) +
  scale_y_continuous(limits = c(0, 1)) +
...
sttr_icc_table <- data.frame(
                           ...
                           icc(LDDM_do2_irr[sttr_hddm_variables[22:24]])$value,
                           ...
                           make_CI(icc(LDDM_do2_irr[sttr_hddm_variables[22:24]])$lbound,icc(LDDM_do2_irr[sttr_hddm_variables[22:24]])$ubound),
                           ...
                           icc(LDDM_do2_irr[sttr_hddm_variables[22:24]])$lbound,
                           ...
                           icc(LDDM_do2_irr[sttr_hddm_variables[22:24]])$ubound,
...
ggplot(sttr_icc_table[c(1:4,7:8),], aes(y=parameters, x=ICC)) +
  ...
  theme_apa()
```
Across the three sessions of Study 1, drift rate and decision threshold distance both showed comparable ICCs to the NIH Toolbox score, though lower than the raw accuracy (see Table 3).
...
annotate_figure(ern_sttr, left = grid::textGrob("HDDM model parameter", rot = 90, vjust = 1, gp = grid::gpar(cex = 1.3)),
...
```
```{r, fig.width=7.5 fig.height=6}
val_rdoc <- ggarrange(inh + rremove("xylab") + rremove("y.text") + rremove("y.ticks"),
...
### Familiarity
```{r Familiarity}
rdoc_familiarity_table <- read.csv(here("./tables/rdoc_heritability_table.csv"))
```
