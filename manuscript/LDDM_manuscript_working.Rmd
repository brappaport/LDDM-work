...
**During this task, participants are asked to identify the direction of the central symbol in a string of symbols that are either congruent (e.g., \< \< \< \< \<) or incongruent (e.g., \< \< \> \< \<) with the central symbol [see @ridderinkhofArrowTimeAdvancing2021].** Typically, participants are instructed to balance responding correctly with responding quickly.
...
This excludes important information.
For example, participants show a natural tendency to slow after making errors on incongruent trials (termed post-error slowing), which may be tied to their cognitive control abilities [@danielmeierPostErrorAdjustments2011; @schroderOptimizingAssessmentsPosterror2020].
...
We compared the psychometric properties of two DDM parameters to raw accuracy, reaction time (RT) interference, and the NIH Toolbox flanker score across two independent adult samples.
...
Sessions were excluded from within-subject analyses for poor accuracy on the flanker task (i.e., \<50%) or poor EEG data quality (i.e., fewer than 10 artifact-free trials per condition) or missing data.
One participant was excluded across all sessions due to below 50% accuracy on the flanker task.
...
**Participant data was excluded if they had poor accuracy on the flanker task (i.e., \<50%).**
...
In Study 1, 26 participants were excluded for having too few trials, and in Study 2, 11 sessions of data across 7 participants were excluded for having too few trials.
...
In Study 1, `r n_winsorized_ern(LDDM_do3_rdoc, LDDM_do3_rdoc_no_outliers, "FCz_ERN_080")` subject was winsorized1.
...
Specifically, `r n_winsorized(LDDM_do3_rdoc, LDDM_do3_rdoc_no_outliers, "Inhib_time")` inhibition and `r n_winsorized_ern(LDDM_do3_rdoc, LDDM_do3_rdoc_no_outliers, "exec_composite")` set-shifting scores were winsorized.
...
DDM parameters were estimated for participants' flanker task performance using the Hierarchical Drift-Diffusion Model Toolbox [HDDM, version 0.8.0, <http://github.com/hddm-devs/hddm>, @wieckiHDDMHierarchicalBayesian2013].
...
Given prior work with these models [e.g., @aylwardTranslatingRodentMeasure2020; @ossolaEffortfulControlAssociated2021; @zieglerModellingADHDReview2016], the present study fit a model with four estimated parameters (drift rate, boundary separation, non-decision time, starting bias), but focused on two parameters of interest: drift rate (i.e., the slope or speed of evidence accumulation) and boundary separation (i.e., speed-accuracy trade-off, see Figure 1).
**Other parameters were fit, including non-decision time (i.e., time needed for visual processing, motor preparation, and response execution) and response bias (i.e., tendency to make one response over the other). However, analyses focused on drift rate and separation boundary as they are more widely studied DDM parameters in clinical and cognitive neuroscience literature and more directly indicative of cognitive control processes [@robinsonLinkingComputationalModels2023].** That said, several models were also fit to participants' behavior and used to supplement analyses to confirm that findings did not vary substantially depending on model specifications (see Supplement).
**The model selected for primary analyses (i.e. model 2 in the Supplement) showed the best match between posterior fits and observed data in the posterior predictive checks (see Supplement 2), despite having a higher DIC (i.e., worse fit) than other alternative models.**
...
**Correlations, linear multiple regressions, linear mixed effect multiple regressions, and ICCs used listwise deletion of participants when missing values were present.**
...
trial_plateaued <- function(data, measure) {
  data %>%
    mutate(rdiff = {{measure}} - lag({{measure}}),
           trials = trials*2) %>%
...
Examination of split half reliability at increments of 15 trials revealed that drift rate reached above 0.70 at `r trial_above_seventy(spearman_brown_rdoc, rv)` trials and increased until `r trial_plateaued(spearman_brown_rdoc, rv)$trials` trials (SB= `r trial_plateaued(spearman_brown_rdoc, rv)$r`) at which point it plateaued (i.e., $\Delta r$ $\le$ 0.01, Spearman Brown at 330 trials: Study 1 = `r round(spearman_brown_rdoc[spearman_brown_rdoc$trials==165,"rv"],2)`, Study 2 Session 1 = `r round(spearman_brown_d1[spearman_brown_d1$trials==165,"rv"],2)`, Study 2 Session 2 = `r round(spearman_brown_d2[spearman_brown_d2$trials==165,"rv"],2)`, Study 2 Session 3 = `r round(spearman_brown_d3[spearman_brown_d3$trials==165,"rv"],2)`).
Boundary separation reached above 0.70 at `r trial_above_seventy(spearman_brown_rdoc, ra)` trials, and increased until `r trial_plateaued(spearman_brown_rdoc, ra)$trials` trials (`r trial_plateaued(spearman_brown_rdoc, ra)$r`), at which point it plateaued (Spearman Brown at 330 trials: Study 1 = `r round(spearman_brown_rdoc[spearman_brown_rdoc$trials==165,"ra"],2)`, Study 2 Session 1 = `r round(spearman_brown_d1[spearman_brown_d1$trials==165,"ra"],2)`, Study 2 Session 2 = `r round(spearman_brown_d2[spearman_brown_d2$trials==165,"ra"],2)`, Study 2 Session 3 = `r round(spearman_brown_d3[spearman_brown_d3$trials==165,"ra"],2)`).
...
Thus, of the measures, boundary separation showed the most robust split-half reliability to varying numbers of trials, and the fewest trials in order to achieve excellent reliability.

## Test-retest reliability

```{r Test-retest}
sttr_icc_table <- read.csv(here("../tables/sttr_icc_table_123.csv"))[-c(3,4),]
```
...
Significant relationships emerged for the ERN~resid~ and the following measures: NIH Toolbox flanker score, drift rate, and boundary separation.
Drift rate, raw accuracy, RT interference, and the NIH Toolbox flanker scores were entered into multiple regression models as simultaneous predictors of the ERN~resid~ to evaluate incremental validity (i.e., the improvement in predictive ability of DDM parameters beyond existing measurements).
The two DDM parameters (drift rate, boundary separation) were entered in separate multiple regression models because they are purported to measure different processes.

The drift rate was significantly associated with ERN~resid~ amplitude in Study 1 over-and-above ERN~resid~'s association with raw accuracy, RT interference, and the NIH Toolbox flanker scores (see Figure 4).
...
Results were similar for session 2 of Study 2, though the associations between drift rate and ERN~resid~ were non-significant at sessions 1 and 3 (see Supplement).
...
Boundary separation was significantly associated with ERN~resid~ amplitude in Study 1 over-and-above ERN~resid~'s association with raw accuracy, RT interference, and the NIH Toolbox flanker scores (see Figure 4).
Study 2 results were consistent with those of Study 1 (see Supplement).
...
Moreover, in these models, raw accuracy was significantly related to ERN~resid~, such that better performance (i.e., fewer errors) was related to a larger ERN~resid~; however, because raw accuracy and ERN~resid~ were not significantly correlated ($r$ = `r round(acc_ern_r$estimate,3)`, $p$ = `r round(acc_ern_r$p.value,3)`), this is likely due to a suppression effect.

### Neuropsychological measures

Bivariate correlations show significant relationships between neuropsychological assessments of cognitive control and the following flanker task measures: NIH Toolbox flanker score, drift rate, and boundary separation.
...
Drift rate was significantly associated with both neuropsychological measures, over-and-above raw accuracy, RT interference, and the NIH Toolbox flanker scores (see Figure 4).
...
Boundary separation was significantly related to both inhibition and set-shifting tasks over-and-above raw accuracy, RT interference, and the NIH Toolbox flanker scores (see Figure 4).
...
In these models, raw accuracy was significantly related to inhibition and set-shifting, such that better performance (i.e., fewer errors) was related to worse inhibition and set-shifting, a counter-intuitive and unexpected effect[^1].

[^1]: The bivariate associations between raw accuracy and ERN~resid~ ($r$ = `r round(acc_ern_r$estimate,3)`, $p$= `r round(acc_ern_r$p.value,3)`), inhibition ($r$ = `r round(acc_inhib_r$estimate,3)`, $p$= `r round(acc_inhib_r$p.value,3)`), and set-shifting ($r$ = `r round(acc_exec_r$estimate,3)`, $p$ = `r round(acc_exec_r$p.value,3)`) measures were non-significant, yet yielded a significant negative association with neuropsychological measures when entered in multiple regressions with drift rate, RT interference, and the NIH Toolbox flanker score, a pattern of results likely due to a suppression effect.
...
LDDM_do3_rdoc_no_outliers_sib <- LDDM_do3_rdoc_no_outliers %>%
  pivot_wider(id_cols=F_ID, names_from=S_ID, 
              values_from=c(B11_avtz_v_z, 
                            B11_avtz_a_z,B11_avtz_t_z,B11_avtz_z_z,flanker_score_rdoc_z,accuracy_incongruent_z,accuracy_z,rt_interference_z))
```

**Analyses of familiality (N = `r icc(LDDM_do3_rdoc_no_outliers_sib[c("B11_avtz_v_z_1","B11_avtz_v_z_2")])$subjects` sibling pairs) showed that RT interference was significantly familial (i.e., similar between siblings), while drift rate was familial at trend levels, boundary separation, NIH Toolbox flanker score, and raw accuracy were not significantly familial (Table 4).**
...
We also found that boundary separation was related to neuropsychological performance.
...
Additionally, as drift rate exhibited significant test-retest reliability and trending evidence of familiality, drift rate may reflect a trait-like indicator of cognitive control.
...
Although the DDM parameters of interest (drift rate and boundary separation) exhibited greater incremental validity than the traditional measures and good split-half reliability, the test-retest reliabilities were still below "good" stability.
...
On the one hand, the moderate test-retest reliability makes it more challenging to assert that measures such as drift rate or boundary separation represent purely trait-like indicators of cognitive control.
On the other hand, DDM parameters may arise from both trait (stable) and state (instable) factors, making them useful indicators to test how changes in cognitive control track changes in psychopathology [@sharpFormalModelsPsychopathological2020].
...
One surprising finding was the relatively good test-retest reliability of raw accuracy and reaction time interference, which were better than drift rate and the NIH Toolbox flanker score.
...
It did not, however, account for unique variance over-and-above DDM parameters at predicting indicators of cognitive control (i.e., ERN~resid~ and neuropsychological performance).
...
Second, we present results from only one version of the DDM model, and alternative models are in the supplement.
The primary model was chosen based on prior literature and because it showed acceptable fit with this data [@aylwardTranslatingRodentMeasure2020; @ossolaEffortfulControlAssociated2021; @zieglerModellingADHDReview2016].
...
**Fifth, given the wide age range of participants in Study 2 (18 to 60-years-old), it is possible that our measure of test-retest reliability was affected by developmental differences. That is, certain measures may show better test-retest reliability at certain ages. Future longitudinal studies of participants of different ages could help fill this gap in the literature.** **Sixth, it is possible that the feedback participants received during the task to respond faster or respond more accurately led them to adopt wider or narrower boundaries which they otherwise might not have.**

In conclusion, this study found that that DDM parameters of drift rate and boundary separation from the flanker task have comparable split-half and test-retest reliability to raw accuracy, RT interference, and the NIH Toolbox flanker score measure, but had better incremental, convergent validity, showing significant relationships with neural (ERN~resid~) and neuropsychological validators (inhibition, set-shifting), and demonstrating trending-levels of familiality.
