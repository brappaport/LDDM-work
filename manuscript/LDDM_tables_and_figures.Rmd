...
demo_rdoc <- FullTable1(vars=c("Gender","Age","accuracy","accuracy_congruent","accuracy_incongruent","rt_interference","flanker_score_rdoc","Pred_FSIQ","Inhib_time","exec_composite_pos"), 
...
sttr_correlations <- correlation_matrix(LDDM_do2_sttr_not_outliers[c(all_measures_sttr)], type = c("pearson"), use = 'lower', replace_diagonal=TRUE, replacement="")
...
sttr_correlations <-  as.data.frame(sttr_correlations) %>%
  mutate(measures=rep(c("ERN","NIH","Acc","RT","DR","DR c","DR i","BS"),3)) %>%
...
rownames(rdoc_correlations)=rep(c("ERN\\textsubscript{r}","NIH","Acc","RT",
                   "DR","DR c","DR i","BS","Inhibition","SS"),1)
```
<!-- \begin{landscape} -->
...
apa_table(rdoc_correlations[,-10],
           row.names=TRUE,
           align = c("p{2.6cm}", rep("p{1.6cm}", ncol(rdoc_correlations)-1)),
           col.names=c("Measures","ERN\\textsubscript{r}","NIH","Acc","RT",
                   "DR","DR\\textsubscript{c}","DR\\textsubscript{i}","BS","Inhibition"),
           font_size="footnotesize",
           caption = "Bivariate pearson correlations among Study 1 variables",
           note = "** p < .01, * p < .05. ERN\\textsubscript{r}: error-related negativity residualized score, NIH: NIH Toolbox flanker score, Acc: raw accuracy (incongruent), RT: reaction time interference, DR\\textsubscript{c}: drift rate on congruent trials, DR\\textsubscript{i}: drift rate on incongruent trials, BS: boundary separation, SS: set-shifting.",
           landscape=TRUE,
           escape=FALSE
)
  
apa_table(sttr_correlations[,-25],
           row.names=FALSE,
           align = c("p{1.7cm}", rep("p{1cm}", ncol(sttr_correlations)-1)),
           col.names=c("Measures",rep(c("ERN\\textsubscript{r}","NIH","Acc","RT",
                   "DR","DR\\textsubscript{c}","DR\\textsubscript{i}","BS"),2),
                   c("ERN\\textsubscript{r}","NIH","Acc","RT",
                   "DR","DR\\textsubscript{c}","DR\\textsubscript{i}")),
           font_size="tiny",
           col_spanners = list(` ` = c(1,1),`Session 1` = c(2,9), `Session 2` = c(10,17), `Session 3` = c(18,24)),
           caption = "Bivariate pearson correlations among Study 2 variables",
           note = "** p < .01, * p < .05. ERN\\textsubscript{r}: error-related negativity residualized score, NIH: NIH Toolbox, Acc: raw accuracy (incongruent), RT: reaction time interference, DR\\textsubscript{c}: drift rate on congruent trials, DR\\textsubscript{i}: drift rate on incongruent trials, BS: boundary separation.",
           stub_indents=list(`Session 1`=c(1:8), `Session 2`=c(9:16), `Session 3`=c(17:24)),
           landscape=TRUE,
           escape=FALSE
)
```
...
make_splithalf_datatable <- function (df) {
  data.frame(trials=seq(15,165,15),
                             rv=rowMeans(df[,c("avtz_DO_v_vcon","avtz_DO_v_vinc")]),
                             rv_cilower=rowMeans(df[,c("avtz_DO_v_v_con_ci_lower","avtz_DO_v_v_inc_ci_lower")]),
                             rv_ciupper=rowMeans(df[,c("avtz_DO_v_v_con_ci_upper","avtz_DO_v_v_inc_ci_upper")]),
                             ra=df$avtz_DO_v_a,
...
spearman_brown_d1_plot <- spearman_brown_d1 %>%
  ...
  select(-c("rdriftcon","rdriftincon","rdrift","rboundary_separation")) %>%
...
spearman_brown_d2_plot <- spearman_brown_d2 %>%
  ...
  select(-c("rdriftcon","rdrift","rboundary_separation")) %>%
  pivot_longer(-c(trials), names_to="Measure") %>%
  separate(Measure, "_", into = c("Measure", "Stat")) %>%
  pivot_wider(names_from=Stat, values_from=value) %>%
  mutate(r=`NA`) %>%
...
ggplot(sttr_icc_table[c(1,4,7:9),], aes(y=parameters, x=ICC, color=parameters)) +
  geom_point() +
...
ern_rdoc <- ggplot(mr_ERN_table_rdoc_raw_new[c(1:4, 13:16),], aes(x=est, label=parameters, y=measure_new, color=measure_new)) +
...
inh <- ggplot(mr_Inhib_table_rdoc_cov_raw_new[c(1:4, 13:16),], aes(x=est, label=parameters, y=measure_new, color=measure_new)) +
  geom_point(position=position_dodge(width = 1), stat = "identity") +
  geom_errorbar(aes(xmin=lci, xmax=uci), width=.1,  position=position_dodge(width = 1), stat="identity") +
  geom_vline(xintercept = 0) +
...
ss <- ggplot(mr_Exec_table_rdoc_cov_raw_new[c(1:4, 13:16),], aes(x=est, label=parameters, y=measure_new, color=measure_new)) +
  geom_point(position=position_dodge(width = 1), stat = "identity") +
  geom_errorbar(aes(xmin=lci, xmax=uci), width=.1,  position=position_dodge(width = 1), stat="identity") +
  geom_vline(xintercept = 0) +
...
\newpage
\begin{landscape}
```{r, fig.cap = paste0("Multiple regression of behavioral measures predicting ERN and neuropsychological tests of inhibition and set-shifting Note: The bivariate correlation between accuracy and inhibition ($r$ = ",round(acc_inhib_r$estimate,3),", $p$ = ", round(acc_inhib_r$p.value,3),") and between accuracy and set-shifting ($r$ = ",round(acc_exec_r$estimate,3),", $p$ = ", round(acc_exec_r$p.value,3),") are non-significant, suggesting the presence of a suppression effect."), out.width='1\\linewidth', fig.width=10, fig.height=8}
...
```{r}
inh_acc_cor <- corr.test(LDDM_do3_rdoc_no_outliers$Inhib_time_rev_z, LDDM_do3_rdoc_no_outliers$accuracy_incongruent_z, method="pearson")
exec_acc_cor <- corr.test(LDDM_do3_rdoc_no_outliers$exec_composite_z, LDDM_do3_rdoc_no_outliers$accuracy_incongruent_z, method="pearson")
```
...
rdoc_heritability_table <- rdoc_heritability_table[-c(4:6),]
rdoc_heritability_table[,1] <- c("Drift rate","Drift rate (congruent)","Drift rate (incongruent)","Boundary separation", "NIH Toolbox", "Raw accuracy", "RT interference")
