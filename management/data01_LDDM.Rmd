...
LDDM_cleaning02_frac_prak_latency <- read.csv("/Volumes/fsmresfiles/PBS/Stewarts/State Trait Study/current_studies/Letkiewicz_DDM/eeg_processing/erppath/frac_peak_latency_p3_200800.csv")[,1:15]
LDDM_cleaning02_frac_prak_latency <- LDDM_cleaning02_frac_prak_latency %>% 
  rename_with(~paste0(., "_fpl"), contains("bin"))

LDDM_cleaning02_mean_amplitude_ERN_CRN_080 <- read.csv("/Volumes/fsmresfiles/PBS/Stewarts/State Trait Study/current_studies/Letkiewicz_DDM/eeg_processing/erppath/mean_amplitude_ERN_CRN_080.csv")[,1:15]
...
LDDM_cleaning02_mean_amplitude_ERN_CRN_0100 <- read.csv("/Volumes/fsmresfiles/PBS/Stewarts/State Trait Study/current_studies/Letkiewicz_DDM/eeg_processing/erppath/mean_amplitude_ERN_CRN_0100.csv")[,1:15]
...
for (ex in electrodes_list[4:6]){
      for (days in sessions) {
        eval(parse(text=paste0('LDDM_cleaning03_d',days,'$',ex,'_P3_onset_incon <- stdres(lm(Incongruent_',ex,'_fpl ~ Congruent_',ex,'_fpl, LDDM_cleaning03_d',days,', na.action=na.exclude))')))
        eval(parse(text=paste0('LDDM_cleaning03_d',days,'$',ex,'_P3_onset_con <- stdres(lm(Congruent_',ex,'_fpl ~ Incongruent_',ex,'_fpl, LDDM_cleaning03_d',days,', na.action=na.exclude))')))
      }
}
```
