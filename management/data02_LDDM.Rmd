...
```{r}
LDDM_cleaning02_DDM <- read.csv("/Volumes/fsmresfiles/PBS/Stewarts/State Trait Study/current_studies/Letkiewicz_DDM/DDM_Results/Flanker_StTr_S1_Blocks_acc_avtz_DO_v.csv")
LDDM_cleaning02_DDM$ID <- LDDM_cleaning02_DDM$id
...
LDDM_cleaning02 <- LDDM_cleaning02_DDM %>%
  left_join(LDDM_cleaning02_erp, by='ID')

save(LDDM_cleaning02, file=here("./data/LDDM_cleaning02.RData"))
```

### 1.5.1 Non-DMM Flanker behavioral scores
```{r}
LDDM_cleaning02_rt <- read_sav("./data/**STTR_master_6.6.18_clean_ERN_PE_LPPbehav**.sav")
...
for (ex in electrodes_list[1:3]){ 
  for (tw in time_windows){ 
    eval(parse(text=paste0('LDDM_cleaning03$',ex,'_ERN_',tw,' <- stdres(lm(Error_',ex,'_',tw,' ~ Correct_',ex,'_',tw,', LDDM_cleaning03, na.action=na.exclude))')))
  }
}

# for (ex in electrodes_list[4:6]){ 
#     eval(parse(text=paste0('LDDM_cleaning03$',ex,'_P3_onset <- stdres(lm(Incongruent_',ex,'_fpl ~ Congruent_',ex,'_fpl, LDDM_cleaning03, na.action=na.exclude))')))
# }
```

# 3. Select only days 1 and 2
```{r}
