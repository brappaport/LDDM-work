...
i=1
mr_INHIB_table_rdoc_nocov <- as.data.frame(matrix(nrow=4, ncol=4))
mr_INHIB_table_rdoc_nocov[,1] <- c("Drift rate","Boundary separation","Non-decision time","Starting bias")

mr_INHIB_table_rdoc_cov <- as.data.frame(matrix(nrow=4, ncol=4))
mr_INHIB_table_rdoc_cov[,1] <- c("Drift rate","Boundary separation","Non-decision time","Starting bias")
mr_INHIB_table_rdoc_cov_raw <- as.data.frame(matrix(nrow=4, ncol=4))
mr_INHIB_table_rdoc_cov_raw[,1] <- c("Drift rate","Boundary separation","Non-decision time","Starting bias")
...
i=1
mr_EXEC_table_rdoc_nocov <- as.data.frame(matrix(nrow=4, ncol=4))
mr_EXEC_table_rdoc_nocov[,1] <- c("Drift rate","Boundary separation","Non-decision time","Starting bias")

mr_EXEC_table_rdoc_cov <- as.data.frame(matrix(nrow=4, ncol=4))
mr_EXEC_table_rdoc_cov[,1] <- c("Drift rate","Boundary separation","Non-decision time","Starting bias")
mr_EXEC_table_rdoc_cov_raw <- as.data.frame(matrix(nrow=4, ncol=4))
mr_EXEC_table_rdoc_cov_raw[,1] <- c("Drift rate","Boundary separation","Non-decision time","Starting bias")
...
LDDM_do3_rdoc_sib <- LDDM_do3_rdoc %>%
  pivot_wider(id_cols=F_ID, names_from=S_ID, 
              values_from=c(B11_avtz_v_z,B11_avtz_a_z,B11_avtz_t_z,B11_avtz_z_z,flanker_score_rdoc_z,accuracy_incongruent_z,accuracy_z,rt_interference_z))

LDDM_heritability_table <- data.frame(parameters= c("v", "a", "t", "z", 
                                                    "nihtoolbox_flanker",
                                                    "accuracy_incongruent_z",
                                                    "rt_interference_z"),
                           ICC=c(icc(LDDM_do3_rdoc_sib[c("B11_avtz_v_z_1","B11_avtz_v_z_2")])$value,
                           icc(LDDM_do3_rdoc_sib[c("B11_avtz_a_z_1","B11_avtz_a_z_2")])$value,
                           icc(LDDM_do3_rdoc_sib[c("B11_avtz_t_z_1","B11_avtz_t_z_2")])$value,
                           icc(LDDM_do3_rdoc_sib[c("B11_avtz_z_z_1","B11_avtz_z_z_2")])$value,
                           ...
                           p=c(icc(LDDM_do3_rdoc_sib[c("B11_avtz_v_z_1","B11_avtz_v_z_2")])$p.value,
                           icc(LDDM_do3_rdoc_sib[c("B11_avtz_a_z_1","B11_avtz_a_z_2")])$p.value,
                           icc(LDDM_do3_rdoc_sib[c("B11_avtz_t_z_1","B11_avtz_t_z_2")])$p.value,
                           icc(LDDM_do3_rdoc_sib[c("B11_avtz_z_z_1","B11_avtz_z_z_2")])$p.value,
...
nsim <- 1000
perm <- data.frame(B11_avtz_v_z_1 = LDDM_do3_rdoc_sib_no_NA$B11_avtz_v_z_1)
res <- data.frame(value=NA) ## set aside space for results

LDDM_do3_rdoc_sib_no_NA <- LDDM_do3_rdoc_sib %>%
  filter(complete.cases(B11_avtz_v_z_1) & complete.cases(B11_avtz_v_z_2))

for (i in 1:nsim) {
    ## standard approach: scramble response value
    perm$B11_avtz_v_z_2_shuffled <- sample(LDDM_do3_rdoc_sib_no_NA$B11_avtz_v_z_2)
    ## compute & store difference in means; store the value
    res[i,1] <- icc(perm[c("B11_avtz_v_z_1","B11_avtz_v_z_2_shuffled")])$value
    # res[i,2] <- icc(perm[c("B11_avtz_v_z_1","B11_avtz_v_z_2_shuffled")])$p.value
}
obs <- icc(LDDM_do3_rdoc_sib[c("B11_avtz_v_z_1","B11_avtz_v_z_2")])$value
...
for (s in seq(1,11)){
    eval(parse(text=paste0('
r_v_half_B',s,' <- cor(rdoc_ddm_splithalf$v_D1_B',s,'a, rdoc_ddm_splithalf$v_D1_B',s,'b, method="pearson", use="pairwise")
r_v_half_ci_B',s,' <- ci_cor(rdoc_ddm_splithalf$v_D1_B',s,'a, rdoc_ddm_splithalf$v_D1_B',s,'b, method="pearson", type = "bootstrap", R=1000)
...
r_a_half_B',s,' <- cor(rdoc_ddm_splithalf$a_D1_B',s,'a, rdoc_ddm_splithalf$a_D1_B',s,'b, method="pearson", use="pairwise")
r_a_half_ci_B',s,' <- ci_cor(rdoc_ddm_splithalf$a_D1_B',s,'a, rdoc_ddm_splithalf$a_D1_B',s,'b, method="pearson", type = "bootstrap", R=1000)
...
for (s in seq(15,165,15)){
  eval(parse(text=paste0('
                         spearman_brown_rdoc[i,8] <- round(as.numeric(r_sb_',s,'),3)
                         spearman_brown_rdoc[i,9] <- round(as.numeric(r_sb_cilower_',s,'),3)
                         spearman_brown_rdoc[i,10] <- round(as.numeric(r_sb_ciupper_',s,'),3)
                         spearman_brown_rdoc[i,11] <- round(as.numeric(racc_sb_',s,'),3)
                         spearman_brown_rdoc[i,12] <- round(as.numeric(racc_sb_cilower_',s,'),3)
                         spearman_brown_rdoc[i,13] <- round(as.numeric(racc_sb_ciupper_',s,'),3)
                         spearman_brown_rdoc[i,14] <- round(as.numeric(rrt_sb_',s,'),3)
                         spearman_brown_rdoc[i,15] <- round(as.numeric(rrt_sb_cilower_',s,'),3)
                         spearman_brown_rdoc[i,16] <- round(as.numeric(rrt_sb_ciupper_',s,'),3)')))
  i=i+1
}
```
...
all_measures_rdoc <- c("FCz_ERN_080_z","flanker_score_rdoc_z","accuracy_z","accuracy_incongruent_z","accuracy_congruent_log_z", "rt_interference_z", "exec_composite_z", "Inhib_time_rev_z",
                  "B11_avtz_v_z","B11_avtz_a_z","B11_avtz_t_z","B11_avtz_z_z")
...
# ggplot(filter(LDDM_do3_rdoc,FCz_ERN_080>-3 & FCz_ERN_080<3), aes(x=FCz_ERN_080_z, y=B11_avtz_v_z)) +
#   geom_point() +
...
# SOBP Stats
```{r, eval=FALSE}
sum(!is.na(LDDM_do3_rdoc_no_outliers$B11_avtz_v_z))

summary(lmer(FCz_ERN_080_z ~ B11_avtz_v_con_z + flanker_score_rdoc_z + accuracy_z + (1 | F_ID), LDDM_do3_rdoc_no_outliers))
summary(lmer(FCz_ERN_080_z ~ B11_avtz_v_incon_z + flanker_score_rdoc_z + accuracy_z + (1 | F_ID), LDDM_do3_rdoc_no_outliers))

summary(lmer(FCz_ERN_080_z ~ B11_avtz_v_z + (1 | F_ID), LDDM_do3_rdoc_no_outliers))
```
