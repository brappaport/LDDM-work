...
LDDM_do3_rdoc_gender <- read_sav(here("./work/data/FINAL_RDoC_Proband_Sibling_SCID_categoricalDx,10.5.2018.sav"))
...
LDDM_do3_rdoc$exec_composite <- -1*rowMeans(LDDM_do3_rdoc[,c("FF_cor","Switch_cor","NL_time_rev","InSw_time_rev")], na.rm=F)
...
var_list <- c("B11_avtz_DO_v_v","B11_avtz_DO_v_v_con","B11_avtz_DO_v_v_incon","B11_avtz_DO_v_a","B11_avtz_DO_v_t","B11_avtz_DO_v_z",
              ...
              "Inhib_time","exec_composite","Pred_FSIQ","Motor_time","accuracy","accuracy_congruent_log","accuracy_incongruent","rt_interference")
...
colnames(mr_INHIB_table_rdoc_nocov) <- c("Parameters","Drift rate", "NIH Toolbox", "Raw accuracy (incongruent)", "RT Interference")
colnames(mr_INHIB_table_rdoc_cov) <- c("Parameters","Drift rate", "NIH Toolbox", "Raw accuracy (incongruent)", "RT Interference")
...
colnames(mr_EXEC_table_rdoc_nocov) <- c("Parameters","Drift rate", "NIH Toolbox", "Raw accuracy (incongruent)", "RT Interference")
colnames(mr_EXEC_table_rdoc_cov) <- c("Parameters","Drift rate", "NIH Toolbox", "Raw accuracy (incongruent)", "RT Interference")
...
LDDM_heritability_table <- data.frame(parameters= c("v", "v_congruent", "v_incongruent", "a", "t", "z", "nihtoolbox_flanker", "accuracy_incongruent_z","accuracy_z","rt_interference_z"),
                           ...
                           icc(LDDM_do3_rdoc_sib[c("rt_interference_z_1","rt_interference_z_2")])$value),
                           ...
                           icc(LDDM_do3_rdoc_sib[c("rt_interference_z_1","rt_interference_z_2")])$p.value))
...
nsim <- 1000
perm <- data.frame(B11_avtz_DO_v_v_z_1 = LDDM_do3_rdoc_sib_no_NA$B11_avtz_DO_v_v_z_1)
res <- data.frame(value=NA) ## set aside space for results

LDDM_do3_rdoc_sib_no_NA <- LDDM_do3_rdoc_sib %>%
  filter(complete.cases(B11_avtz_DO_v_v_z_1) & complete.cases(B11_avtz_DO_v_v_z_2))

for (i in 1:nsim) {
    ## standard approach: scramble response value
    perm$B11_avtz_DO_v_v_z_2_shuffled <- sample(LDDM_do3_rdoc_sib_no_NA$B11_avtz_DO_v_v_z_2)
    ## compute & store difference in means; store the value
    res[i,1] <- icc(perm[c("B11_avtz_DO_v_v_z_1","B11_avtz_DO_v_v_z_2_shuffled")])$value
    # res[i,2] <- icc(perm[c("B11_avtz_DO_v_v_z_1","B11_avtz_DO_v_v_z_2_shuffled")])$p.value
}
obs <- icc(LDDM_do3_rdoc_sib[c("B11_avtz_DO_v_v_z_1","B11_avtz_DO_v_v_z_2")])$value

hist(res, col='grey'); abline(v=obs, lwd=3, col="red")
```
...
for (s in seq(15,165,15)){
  eval(parse(text=paste0('spearman_brown_rdoc[i,2] <- round(as.numeric(r_sb_',s,'),3)
                         ...
                         spearman_brown_rdoc[i,8] <- round(as.numeric(rrt_sb_',s,'),3)
                         spearman_brown_rdoc[i,9] <- round(as.numeric(rrt_sb_cilower_',s,'),3)
                         spearman_brown_rdoc[i,10] <- round(as.numeric(rrt_sb_ciupper_',s,'),3)')))
  i=i+1
}

# rdoc_ddm_splithalf <- read.csv(here("./Split_Half/RDoC_splithalf.csv"), header=TRUE)
rdoc_ddm_splithalf <- read.csv(here("../data/Flanker_RDoC_SplitHalf_Blocks_avtz.csv"), header=TRUE)

spearman_brown_rdoc$rdriftcon <- rdoc_ddm_splithalf$avtz_DO_v_vcon
...
```{r}
all_measures_rdoc <- c("FCz_ERN_080_z","flanker_score_rdoc_z","accuracy_z","accuracy_incongruent_z","accuracy_congruent_log_z", "rt_interference_z", "exec_composite_z", "Inhib_time_rev_z",
